...
//-------------------------------------------------------
void UtilsStd :: writeStr(string s, string filename)
{
  unsigned int i;
  /*char* path = new char [filename.length() + 1];
  for ( i = 0; i < filename.length(); i++)
...
/** Compress a STL string using zlib with given compression level and return
  * the binary data. */
std::string UtilsStd::compress_string(const std::string& str,     int compressionlevel)
{
...
std::string UtilsStd::decompress_string(const std::string& str)
{
    z_stream zs;                        // z_stream is zlib's control structure
    memset(&zs, 0, sizeof(zs));
